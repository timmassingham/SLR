...
/*
 * umd Must include math.h before f2c.h - f2c does a #define abs. (Thanks go
 * to Martin Wauchope for providing this correction) We manually included
 * AT&T's f2c.h in this source file, i.e. it does not have to be present
 * separately in order to compile.
 */
#include <math.h>
 ...
 * NOTICE !!!!
 * 
 * 1. The routines contained in this file are due to Prof. K.Schittkowski of the
 * University of Bayreuth, Germany (modification of routines due to Prof. MJD
 * Powell at the University of Cambridge).  They can be freely distributed.
 * 
 * 2. A few minor modifications were performed at the University of Maryland.
 * They are marked in the code by "umd".
 * 
 * A.L. Tits, J.L. Zhou, and Craig Lawrence University of Maryland
 * 
**********************************************************************
 * 
 * 
 ...
 * 
 * MINIMIZE        .5*X'*C*X + D'*X SUBJECT TO      A(J)*X  +  B(J)   =  0  ,
 * J=1,...,ME A(J)*X  +  B(J)  >=  0  ,  J=ME+1,...,M XL  <=  X  <=  XU
 * 
 ...
 * SITUATION IS INDICATED BY IWAR(1)=1. ALTERNATIVELY, I.E. IF IWAR(1)=0, THE
 * OBJECTIVE FUNCTION MATRIX CAN ALSO BE PROVIDED IN FACTORIZED FORM. IN THIS
 * CASE, C IS AN UPPER TRIANGULAR MATRIX.
 * 
 * THE SUBROUTINE REORGANIZES SOME DATA SO THAT THE PROBLEM CAN BE SOLVED BY A
 * MODIFICATION OF AN ALGORITHM PROPOSED BY POWELL (1983).
 * 
 * 
 * USAGE:
 * 
 ...
 * 
 * M :        TOTAL NUMBER OF CONSTRAINTS. ME :       NUMBER OF EQUALITY
 * CONSTRAINTS. MMAX :     ROW DIMENSION OF A. MMAX MUST BE AT LEAST ONE AND
 * GREATER THAN M. N :        NUMBER OF VARIABLES. NMAX :     ROW DIMENSION
 * OF C. NMAX MUST BE GREATER OR EQUAL TO N. MNN :      MUST BE EQUAL TO M +
 * N + N. C(NMAX,NMAX): OBJECTIVE FUNCTION MATRIX WHICH SHOULD BE SYMMETRIC
 * AND POSITIVE DEFINITE. IF IWAR(1) = 0, C IS SUPPOSED TO BE THE
 * CHOLESKEY-FACTOR OF ANOTHER MATRIX, I.E. C IS UPPER TRIANGULAR. D(NMAX) :
 * CONTAINS THE CONSTANT VECTOR OF THE OBJECTIVE FUNCTION. A(MMAX,NMAX):
 * CONTAINS THE DATA MATRIX OF THE LINEAR CONSTRAINTS. B(MMAX) :  CONTAINS
 * THE CONSTANT DATA OF THE LINEAR CONSTRAINTS. XL(N),XU(N): CONTAIN THE
 * LOWER AND UPPER BOUNDS FOR THE VARIABLES. X(N) :     ON RETURN, X CONTAINS
 * THE OPTIMAL SOLUTION VECTOR. U(MNN) :   ON RETURN, U CONTAINS THE LAGRANGE
 * MULTIPLIERS. THE FIRST M POSITIONS ARE RESERVED FOR THE MULTIPLIERS OF THE
 * M LINEAR CONSTRAINTS AND THE SUBSEQUENT ONES FOR THE MULTIPLIERS OF THE
 * LOWER AND UPPER BOUNDS. ON SUCCESSFUL TERMINATION, ALL VALUES OF U WITH
 * RESPECT TO INEQUALITIES AND BOUNDS SHOULD BE GREATER OR EQUAL TO ZERO.
 * IOUT :     INTEGER INDICATING THE DESIRED OUTPUT UNIT NUMBER, I.E. ALL
 * WRITE-STATEMENTS START WITH 'WRITE(IOUT,... '. IFAIL :    SHOWS THE
 * TERMINATION REASON. IFAIL = 0 :   SUCCESSFUL RETURN. IFAIL = 1 :   TOO
 * MANY ITERATIONS (MORE THAN 40*(N+M)). IFAIL = 2 :   ACCURACY INSUFFICIENT
 * TO SATISFY CONVERGENCE CRITERION. IFAIL = 5 :   LENGTH OF A WORKING ARRAY
 * IS TOO SHORT. IFAIL > 10 :  THE CONSTRAINTS ARE INCONSISTENT. IPRINT :
 * OUTPUT CONTROL. IPRINT = 0 :  NO OUTPUT OF QL0001. IPRINT > 0 :  BRIEF
 * OUTPUT IN ERROR CASES. WAR(LWAR) : REAL WORKING ARRAY. THE LENGTH LWAR
 * SHOULD BE GRATER THAN 3*NMAX*NMAX/2 + 10*NMAX + 2*MMAX. IWAR(LIWAR):
 * INTEGER WORKING ARRAY. THE LENGTH LIWAR SHOULD BE AT LEAST N. IF IWAR(1)=0
 * INITIALLY, THEN THE CHOLESKY DECOMPOSITION WHICH IS REQUIRED BY THE DUAL
 * ALGORITHM TO GET THE FIRST UNCONSTRAINED MINIMUM OF THE OBJECTIVE
 * FUNCTION, IS PERFORMED INTERNALLY. OTHERWISE, I.E. IF IWAR(1)=1, THEN IT
 * IS ASSUMED THAT THE USER PROVIDES THE INITIAL FAC- TORIZATION BY HIMSELF
 * AND STORES IT IN THE UPPER TRIAN- GULAR PART OF THE ARRAY C.
 * 
 * A NAMED COMMON-BLOCK  /CMACHE/EPS   MUST BE PROVIDED BY THE USER, WHERE EPS
 * DEFINES A GUESS FOR THE UNDERLYING MACHINE PRECISION.
 * 
 * 
 * AUTHOR:    K. SCHITTKOWSKI, MATHEMATISCHES INSTITUT, UNIVERSITAET BAYREUTH,
 * 8580 BAYREUTH, GERMANY, F.R.
 * 
 * 
...
int 
ql0002_(integer * n, integer * m, integer * meq, integer * mmax,
...
#ifdef __STDC__
int 
ql0001_(int *m, int *me, int *mmax, int *n, int *nmax, int *mnn,
...
/* Subroutine */
int 
ql0001_(m, me, mmax, n, nmax, mnn, c, d, a, b, xl, xu, x,
...
/*
 * umd Two alternative definitions are provided in order to give ANSI
 * compliance. (Thanks got to Martin Wauchope for providing this correction)
 */
#ifdef __STDC__
int 
ql0002_(integer * n, integer * m, integer * meq, integer * mmax,
...
#else
 /* Subroutine */ int 
ql0002_(n, m, meq, mmax, mn, mnn, nmax, lql, a, b, grad,
...
{
	/* System generated locals */
	integer         a_dim1, a_offset, g_dim1, g_offset, i__1, i__2,
	                i__3, i__4;
	...
	static integer  ii, il, kk, jl, ip, ir, nm, is, iu, iw, ju, ix,
	                iz, nu, iy;
