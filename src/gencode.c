 ...
 * Should really be autogenerated from genetic code.
 */
int 
CodonToQcoord(const int codon, const enum SEQTYPE seqtype, const int gencode)
{
	int             mod;
	assert(SEQTYPE_CODON==seqtype);
	assert(IsValidGencode(gencode));
	assert(IsValidBase(codon, seqtype, gencode));

	/*GapChar is same for both SEQTYPE_CODON and SEQTYPE_CODONQ*/
	if (GapChar(seqtype) == codon)
		return codon;
