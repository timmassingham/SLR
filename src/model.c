...
void CalculateF(double *F, const double *d, double *space, const double factor,
                const int n);
...
void
MakeQ_From_S_MGfreq(double *mat, const double *pi, const int n,
                    const int gencode)
{
    int i, j;
    ...
    for (i = 0; i < n; i++) {
        ...
        for (j = 0; j < n; j++) {
            ...
            if ((diff & 3) != 0) {
                ...
                err(EXIT_FAILURE,
                    "Impossible condition in MakeQ_From_S_MGfreq %s:%d\n",
                    __FILE__, __LINE__);
            }
...
int MakeSym_From_Q(double *mat, const double *pi, const int n)
{
    int i, j;
    assert(NULL != mat);
    assert(NULL != pi);
...
extern double t[];
void
MakeFactQ_FromFactSym(double *ev, double *inv_ev, double *v, const double *pi,
                      const int n)
{
    int i, j;
    assert(NULL != ev);
                         ...
                         const double *q)
{
    ...
    if (length * rate * scale < -DBL_EPSILON) {
        err(EXIT_FAILURE,
            "Error. lrs less than zero. len=%e, rate=%e scale=%e\n", length,
            rate, scale);
    }
...
double *GetP(MODEL * model, const double length, double *mat)
{
    ...
    if (!model->factorized) {
        ...
        model->space = tmp;
        MakeFactQ_FromFactSym(model->ev, model->inv_ev, model->v, model->pi,
                              nbase);
        model->Getq(model);
        model->factorized = 1;

    }
    const double lenfact =
        (Branches_Proportional == model->has_branches) ? model->param[0] : 1.0;
    MakeP_From_FactQ(model->v, model->ev, model->inv_ev, lenfact * length,
                     Rate(model), Scale(model), mat, nbase, model->space,
                     model->pi, model->q);
...
void MakeDerivFromP(MODEL * model, const double blen, double *bmat)
{
    ...
    double *space = model->space;

    const double factor = Scale(model) * Rate(model) * blen;
    const double sfactor =
        (Branches_Proportional == model->has_branches) ? model->param[0] : 1.;
    ...
    Matrix_MatrixT_Mult(model->F, n, n, model->inv_ev, n, n, space);

    Matrix_Matrix_Mult(model->ev, n, n, space, n, n, bmat);
...
/**  Calculate the derivative of the Q matrix

Calculates S dQ S^{-1} where dQ is the derivative of
the Q matrix with respect to a parameter and S is the
inverse eigen vector matrix.  The result is stored as
model->dq. 
 
@param model    Model parameter
@param param    Parameter of which derivative is by
**/
void MakeSdQS(MODEL * model, const int param)
{
    const int n = model->nbase;
    double * tmp = model->space;
...
/**  Calculate the F matrix for 

@param F       Space to store F
@param d       Array of eigen values
@param space   Temporary storage (size n)
@param factor  Scaling factor
@param n       Size of (square) matrices
**/
void
CalculateF(double *F, const double *d, double *space, const double factor,
           const int n)
{
    ...
    for (int i = 0; i < n; i++){
        for (int j = 0; j < i; j++) {
            if (fabs(d[i] - d[j]) > DBL_EPSILON){
                F[i * n + j] = (space[i] - space[j]) / (factor * (d[i] - d[j]));
            } else {
                // Catch case where d[i] == d[j]
                F[i * n + j] = space[i];
            }
        }
        F[i * n + i] = space[i];
    ...
    //  Fill in remainder of matrix using symmetry
    for (int i = 0; i < n; i++){
        for (int j = 0; j < i; j++){
            F[j * n + i] = F[i * n + j];
        }
...
void GetdQ(MODEL * model, const int n, double *q)
{
                      ...
                      const double delta)
{
    ...
    for (int i = 0; i < nparam; i++) {
        ...
        for (int j = 0; j < model->nbase * model->nbase; j++) {
            ...
            p_test[j] /= 2 * delta;
            sumsqrdiff +=
                (p_test[j] - dp_test[i * nbase * nbase + j]) * (p_test[j] -
                                                                dp_test[i *
                                                                        nbase *
                                                                        nbase +
                                                                        j]);
