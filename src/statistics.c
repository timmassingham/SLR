...
double mse_pfdr_deltaapprox_R0(const double lambda, const double gamma,
                               const unsigned int c, const unsigned int m);
double mse_pfdr_deltaapprox(const double lambda, const double gamma,
                            const unsigned int a, const unsigned int b,
                            const unsigned int c, const unsigned int d,
                            const unsigned int m);

/**  Calculate (1 + x) ^ y - 1 in a numerically stable manner
...
static double CorrectPval(const double p, const int n, const int method)
{
    ...
    case SIDAK:
        ...
        break;
    default:
        puts("Unknown method in CorrectPval");
        abort();
    }
...
double *Pvalue_adjust_SingleStep(const double *pval, const int n,
                                 const int method)
{
    assert(IsPvals(pval, n));
    assert(method == BONFERRONI || method == SIDAK);

    double * pval_adj = calloc(n, sizeof(double));
    OOM(pval_adj);
...
double *Pvalue_adjust_StepDown(const double *pval, const int n,
                               const int method)
{
...
double *Pvalue_adjust_StepUp(const double *pval, const int n, const int method)
{
    ...
    if (method == SIDAK) {
        fputs
            ("Using Sidak adjustment with Hochberg\'s step-up proceedure.\nMay not be valid -- check theory!\n",
             stderr);
    }
...
double variance(const VEC v)
{
    ...
    return (sample_variance_naive(v) - correction * correction / len) / (len -
                                                                         1);
}
...
struct summary *fprint_stat_summary(FILE * fp, const char *name,
                                    struct summary *s)
{
    assert(NULL != fp);
    assert(NULL != name);
    ...
    fprintf(fp, "%5.3f %5.3f %5.3f %5.3f %5.3f %5.3f\n", vget(s->quantiles, 0),
            vget(s->quantiles, 1), vget(s->quantiles, 2), s->mean,
            vget(s->quantiles, 3), vget(s->quantiles, 4));
 ...
 */
double pFDR_storey02(const double *pval, const unsigned int m,
                     const double lambda, const double gamma)
{
    ...
    unsigned int r = 0;
    for (unsigned int i = 0; i < m; i++) {
        if (pval[i] - gamma <= gamma * DBL_EPSILON)
            r++;
    }
...
double FDR_storey02(const double *pval, const unsigned int m,
                    const double lambda, const double gamma)
{
    assert(NULL != pval);
    assert(lambda >= 0. && lambda <= 1.);
 ...
 */
double estimate_lambda_deltaapprox(const double *pval, const unsigned int m,
                                   const double gamma)
{
    assert(NULL != pval);
    assert(m > 0);
    ...
    for (unsigned int i = 0; i < m; i++) {
        if (pval_sort[i] - gamma <= gamma * DBL_EPSILON) {
            a--;
            b++;
            assert(d == 0);
        } else {
        ...
        lambda_array[i + 1] =
            (Rgamma != 0) ? mse_pfdr_deltaapprox(pval_sort[i], gamma, a, b, c,
                                                 d, m)
    ...
    for (unsigned int idx = 0; idx < m; idx++) {
        if (lambda_array[idx] < lambda_array[min_idx]) {
            min_idx = idx;
        }
    }
                            ...
                            const unsigned int m)
{
    ...
    return Klambda * Klambda / (apb_sqr * apb_sqr) * (a * bmc * bmc +
                                                      b * apc * apc +
                                                      c * apb_sqr);
}
...
double estimate_lambda_storey04(const double *pval, const unsigned int m)
{
    ...
    for (unsigned int i = 0; i < m - 1; i++) {
        ...
        const double mse = p * (1 - p) / ((1. - lambda) * (1. - lambda))
            + pi_0 * pi_0 - 2 * pi_star * pi_0 + pi_star * pi_star;
        if (mse < mse_min) {
            mse_min = mse;
